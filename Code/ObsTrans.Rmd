...
```{r}
# Exposure
lead <- read_xpt("PB12.XPT")

# Outcome (to be converted to hypertension)
BP <- read_xpt("BP12.XPT")

# Confounders
demo <- read_xpt("DEMO12.XPT")
BMI <- read_xpt("BMI12.XPT")
smoke <- read_xpt("SMOKE12.XPT")
ALC <- read_xpt("ALQ12.XPT")

# Possible EMM
stress <- read_xpt("STRESS12.XPT")
sleep <- read_xpt("SLEEP12.XPT")
```
...
# Data Correction so Never Smokers Don't Automatically Get Dropped
...
## NHANES Smoking Data Transformations
...
```{r}
brfss_original <- read_xpt("BRFSS.XPT")
# brfss

brfss_original
```

# BRFSS Setup - Defining BRFSS

```{r}
brfss_var <- c("_AGE80", "_BMI5", "ALCDAY4", "SEXVAR", "EDUCA", "_SMOKER3", "MENTHLTH", "SLEPTIM1")

brfss <- brfss_original %>%   
  subset(select = all_of(brfss_var)) %>% 
  rename(AGE80 = "_AGE80") %>% 
  rename(BMI = "_BMI5") %>% 
  rename(SMOKER = "_SMOKER3") 
...
The Kolmogorov-Smirnov test indicates there are different distributions in days stressed per month but not sleep duration.
...
However, for the purposes of illustration, we should still consider sleep as a differing variable and stress as an EMM. So we let vz = {stress, sleep}.
...
# Simulate Data (Study and Target Pop.)
...
# Checking out sample ATE and transported ATE (Table 2)
